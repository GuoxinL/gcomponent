...
func (c *Configuration) Initialize(params ...interface{}) interface{} {
	...
	if err != nil {
		logger.Fatal("GComponent [redis]Read configuration exceptions and exit.", zap.Error(err))
	}
	...
	for _, source := range c.RedisSources {
		...
		if err := client.Ping().Err(); err != nil {
			logger.Fatal("GComponent [redis]connection to Redis failed.", zap.Error(err))
		} else {
			logger.Info("GComponent [redis] ping success.")
		}
