...
/**
如果需要返回的error请使用ErrorP方法，会将error panic
*/
...
func Error0(arg0 interface{}, args ...interface{}) {
	const (
		lvl = ERROR
	)
	switch first := arg0.(type) {
...
// These functions will execute a closure exactly once, to build the error message for the return
// Wrapper for (*Logger).Error
func ErrorP(arg0 interface{}, args ...interface{}) {
	const (
		lvl = ERROR
	)
	var err error
	...
	case string:
		// Use the string as a format string
		Global.intLogf(lvl, first, args...)
		err = errors.New(fmt.Sprintf(first, args...))
	case func() string:
		// Log the closure (no other arguments used)
		str := first()
		Global.intLogf(lvl, "%s", str)
	...
	default:
		// Build a format string so that it will be similar to Sprint
		Global.intLogf(lvl, fmt.Sprint(first)+strings.Repeat(" %v", len(args)), args...)
		err = errors.New(fmt.Sprint(first) + fmt.Sprintf(strings.Repeat(" %v", len(args)), args...))
	}
	panic(err)
}
